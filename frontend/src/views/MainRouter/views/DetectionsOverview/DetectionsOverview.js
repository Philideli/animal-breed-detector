...
class DetectionsOverview extends Component {
	...
	handleDelete = (id) => {
		...
		makeApiServiceProxyRequest(`animals/analysis/${id}`, 'delete', undefined,
			(data) => {
				if (data?.success) {
	...
	loadData = (callback) => {
		this.setState({loading: true});
		makeApiServiceProxyRequest(`animals/analyses/overview`, 'get', undefined, 
			(data) => {
	...
	render() {
		...
		const detections = data?.analyses;
							...
							<table className="table table-hover table-light">
								...
								<tbody>
									{Array.isArray(detections) && detections.map((d, i) => (
										<tr key={`row-${i}`}>
											<th scope="row"><Link to={`/app/detection/${d.id}`}>{d.id}</Link></th>
											<td style={filenameStyle}><Link to={`/app/detection/${d.id}`}>{d?.metadata?.filename}</Link></td>
											<td>
												{new Date(d?.metadata?.timestamp).toLocaleDateString()}
												{' '}
												{new Date(d?.metadata?.timestamp).toLocaleTimeString()}
											</td>
											...
											<td>
												...
												>
													...
													<Dropdown.Menu>
														<Link className="dropdown-item d-flex flex-nowrap" to={`/app/detection/${d.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
