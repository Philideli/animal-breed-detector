...
class DetectionDetail extends Component {
	...
	loadData = () => {
		...
		makeApiServiceProxyRequest(`animals/analysis/${this.id}`, 'get', undefined, 
			(data) => {
	...
	handleRerun = () => {
		...
		makeApiServiceProxyRequest(`animals/analysis/${this.id}/rerun`, 'put', JSON.stringify(body),
			(data) => {
				if (data?.success) {
					this.loadData();
	...
	handleDelete = () => {
		this.setState({loading: true});
		makeApiServiceProxyRequest(`animals/analysis/${this.id}`, 'delete', undefined,
			(data) => {
				if (data?.success) {
	...
	render() {
		...
		if (showOriginalImageModal) {
			modalComponent = (
                ...
                >
					<img
						src={getApiUrl(`animals/analysis/${this.id}/image/original`)}
						alt="Original"
						className="img-fluid"
					/>
				</div>
		...
		return (
			<div className="container my-5">
				...
				{redirectToOverview && (<Redirect to={`/app/overview`} />)}
				<div className="card">
						...
						<div className='d-flex justify-content-center'>
							{loading ? 
								(
									...
									<img 
										src={getApiUrl(`animals/analysis/${this.id}/image/result?${new Date().getTime()}`)}
										alt="Result"
